...
from appfc.load_balancing.protocol import create_agent, local_voting, acc_local_voting, generate_queue
...
def start():
    ...
    agents = create_agent(Adj, weights, queue, is_logging=True)
    ...
    sequence = None
    if alg == "LVP":
        sequence, _ = local_voting(agents, parameters, noise_function, num_iterations=steps)
    elif alg == "ALVP":
        sequence, _ = acc_local_voting(agents, parameters, noise_function, num_iterations=steps)
